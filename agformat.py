...
arithmeticop : Add LPAREN RPAREN | Sub LPAREN RPAREN | Mult LPAREN RPAREN | Div LPAREN RPAREN | Mod LPAREN RPAREN | Pow LPAREN RPAREN | LShift LPAREN RPAREN | RShift LPAREN RPAREN | BitOr LPAREN RPAREN | BitXor LPAREN RPAREN | BitAnd LPAREN RPAREN | FloorDiv LPAREN RPAREN
    ...
    with open(sys.argv[2], "w") as aofile:
        for head, productions in rules:
            ...
            for pn,production in enumerate(productions):
                ...
                constructor = prods[0]
                recordname = formatToken(head, constructor) #constructor.upper().replace("_", "")
                ...
                formatedRule = " ".join([formatToken(head, p) for p in prods])
                if pn == 0:
                    print("%s: %s\n%s  { %s }" % 
